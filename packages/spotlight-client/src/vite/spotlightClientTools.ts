...
function schemaFromType(node: t.TSType): JsonSchema {
  ...
  if (t.isTSNullKeyword(node) || t.isTSVoidKeyword(node)) return { type: "null" };
...
function handlerFunction(node: t.Expression | t.SpreadElement | t.JSXNamespacedName | t.ArgumentPlaceholder) {
  if (t.isArrowFunctionExpression(node) || t.isFunctionExpression(node)) return node;
  return null;
}

function inputSchema(fn: t.ArrowFunctionExpression | t.FunctionExpression): JsonSchema {
  ...
  const annotation = t.isTSTypeAnnotation(paramAnnotation)
    ...
    : undefined;
  if (!annotation) throw new Error("handler input parameter needs a TypeScript type");
  return schemaFromType(annotation);
}

function hasValueReturn(fn: t.ArrowFunctionExpression | t.FunctionExpression): boolean {
...
function outputSchema(fn: t.ArrowFunctionExpression | t.FunctionExpression): JsonSchema {
  ...
  throw new Error("handler return type must be explicit when it returns a value");
}
...
function jsDoc(path: NodePath<t.VariableDeclarator>): string {
  ...
  const comment = comments.reverse().find((item) => item.type === "CommentBlock");
  if (!comment) return "";
...
function staticJsonValue(
  ...
  if (t.isIdentifier(node)) {
    if (seen.has(node.name)) throw new Error(`circular schema const: ${node.name}`);
...
function shouldInclude(id: string, include: string | RegExp): boolean {
  ...
  if (!/\.[cm]?[jt]sx?$/.test(clean) || clean.includes("node_modules")) return false;
...
export function transformSpotlightClientTools(
  ...
  if (!code.includes("defineClientTool")) return null;
  ...
  traverseAst(ast, {
    VariableDeclarator(path) {
      ...
      if (call.arguments.length < 1 || call.arguments.length > 2) {
        throw path.buildCodeFrameError("defineClientTool expects handler and optional options");
      }
      const fn = handlerFunction(call.arguments[0]);
      if (!fn) throw path.buildCodeFrameError("defineClientTool handler must be inline");
      ...
      if (optionArg && !t.isObjectExpression(optionArg)) {
        throw path.buildCodeFrameError("Client tool options must be an object literal");
      }
      ...
      if (optionArg) {
        ...
        const gapBeforeClose = code.slice(call.arguments[0].end!, call.end! - 1);
...
export default function spotlightClientTools(
  ...
  return {
    ...
    configResolved(config) {
      productionBuild = config.command === "build" && config.mode === "production";
      if (
        productionBuild &&
