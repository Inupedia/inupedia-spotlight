...
export function prepareRunSkills(
  ...
  for (const raw of skills ?? []) {
    ...
    if (!name || !description || seen.has(name) || raw.disableModelInvocation === true) continue;
    seen.add(name);
    const bodyLimit = Math.min(MAX_SKILL_BODY_CHARS, remainingChars);
    const instruction = compact(raw.skillInstructionBody, bodyLimit);
    const appendixLimit = Math.min(4_000, Math.max(0, remainingChars - (instruction?.length ?? 0)));
    ...
    result.push({
      ...
      allowedTools: raw.allowedTools?.filter((tool) => registeredTools.has(tool)),
      capabilityExamples: raw.capabilityExamples?.map((item) => item.trim()).filter(Boolean).slice(0, 8),
...
export function formatSkillInstructions(skills: SpotlightSkill[]): string {
  if (skills.length === 0) return "No consumer Skills were registered for this run.";
  return skills
    .map((skill) => {
      const lines = [
        ...
        skill.allowedTools?.length ? `Allowed client tools: ${skill.allowedTools.join(", ")}` : "Allowed client tools: none",
...
export function isCapabilityHelpQuestion(
  ...
  const configured = Array.isArray(uiPrompts?.capabilityHelpPatterns)
    ? uiPrompts.capabilityHelpPatterns.filter((item): item is string => typeof item === "string")
    : [];
...
export function buildCapabilityHelp(
  ...
  const skillLines = skills.map((skill) => {
    ...
    const usableTools = (skill.allowedTools ?? []).filter((name) => availableToolNames.has(name));
  ...
  const footer = typeof uiPrompts?.capabilityHelpFooter === "string"
    ? uiPrompts.capabilityHelpFooter.trim()
    : "直接用自然语言告诉我你想查询什么，或要页面执行什么操作。";
  if (skillLines.length > 0) {
    return ["我当前可以协助这些事项：", "", ...skillLines, "", footer].join("\n");
  }
  const toolLines = tools.slice(0, 20).map((tool) => `- ${tool.description || tool.name}`);
